...
keygen_cmd_args( int *    pargc,
  ...
  if( FD_LIKELY( !strcmp( *pargv[ 0 ], "identity" ) ) ) args->keygen.key = KEYGEN_KEY_IDENTITY;
  else if( FD_LIKELY( !strcmp( *pargv[ 0 ], "vote"  ) ) ) args->keygen.key = KEYGEN_KEY_VOTE_ACCOUNT;
...
keygen_cmd_fn( args_t *         args,
  ...
  if( FD_LIKELY( args->keygen.key == KEYGEN_KEY_IDENTITY ) ) {
    generate_keypair( config->consensus.identity_path, config );
  }

  if( FD_LIKELY( args->keygen.key == KEYGEN_KEY_VOTE_ACCOUNT ) ) {
