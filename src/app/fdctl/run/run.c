...
main_pid_namespace( void * _args ) {
  ...
  int allow_fds[ 3+FD_TOPO_MAX_TILES ];
  allow_fds[ 0 ] = 1; /* write end of main pipe */
  allow_fds[ 1 ] = 2; /* stderr */
  allow_fds[ 2 ] = 4; /* logfile */
  for( ulong i=0; i<child_cnt; i++ )
    allow_fds[ 3+i ] = fds[ 1+i ].fd; /* read end of child pipes */
  ...
  fd_sandbox( config->development.sandbox,
              ...
              3+child_cnt, /* Number of fds we are allowing to be mapped into the process */
              allow_fds,
