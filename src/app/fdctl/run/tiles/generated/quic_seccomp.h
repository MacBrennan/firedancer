...
static void populate_sock_filter_policy_quic( ulong out_cnt, struct sock_filter * out, unsigned int netlink_fd) {
  FD_TEST( out_cnt >= 34 );
...
//  lbl_1:
    /* load syscall argument 0 in accumulator */
    BPF_STMT( BPF_LD | BPF_W | BPF_ABS, offsetof(struct seccomp_data, args[0])),
    BPF_JUMP( BPF_JMP | BPF_JEQ | BPF_K, 4, /* RET_ALLOW */ 19, /* RET_KILL_PROCESS */ 18 ),
//  check_fsync:
    /* load syscall argument 0 in accumulator */
    BPF_STMT( BPF_LD | BPF_W | BPF_ABS, offsetof(struct seccomp_data, args[0])),
    BPF_JUMP( BPF_JMP | BPF_JEQ | BPF_K, 4, /* RET_ALLOW */ 17, /* RET_KILL_PROCESS */ 16 ),
//  check_sendto:
    /* load syscall argument 0 in accumulator */
    BPF_STMT( BPF_LD | BPF_W | BPF_ABS, offsetof(struct seccomp_data, args[0])),
